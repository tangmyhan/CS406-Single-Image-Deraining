...
class DataLoaderTrain(Dataset):
    def __init__(self, rgb_dir, img_options=None, subset_ratio=1.0):
        ...
        # GIẢM DATASET
        total = len(self.inp_filenames)
        keep = int(total * subset_ratio)
        self.inp_filenames = self.inp_filenames[:keep]
        self.tar_filenames = self.tar_filenames[:keep]

        print(f"➡ Training with {keep}/{total} images ({subset_ratio*100:.1f}%)")
